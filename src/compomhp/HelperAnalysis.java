...
//		System.out.println("Calculated interesting fields for methods all edges:"+newmethFields.size());
	...
	public static void populateIntFieldsUsingPreAnalysis(Arg arg) {
...
//				}
	...
	private static void genCallGraph(Arg arg){
		Solver.intPass = true;
		solver = Solver.v();
//		Arg arg = new Arg();
//		arg.combined = false;
...
//		 	Set<SootMethod> noField = new HashSet<>();
			...
			while(!edgeList.isEmpty()) {
				...
				SootMethod src_meth = edge.getFirst();
				SootMethod tgt_meth = edge.getSecond();
